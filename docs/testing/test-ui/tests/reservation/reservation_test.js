...
Scenario("TC02 - Kiểm tra lịch sử đặt bàn khi chưa đăng nhập", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
...
Scenario("TC05 - Kiểm tra validation khi bỏ trống họ tên", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
...
Scenario("TC06 - Kiểm tra validation khi bỏ trống số điện thoại", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
...
Scenario("TC07 - Kiểm tra validation khi bỏ trống ngày", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
...
Scenario("TC08 - Kiểm tra validation khi bỏ trống giờ", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
...
Scenario("TC10 - Kiểm tra nhập đầy đủ thông tin đặt bàn", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
  ...
  I.fillField("#note", "Sinh nhật, gần cửa sổ");

  I.seeInField("#fullName", "Trần Minh Trí");

  I.seeInField("#phone", "0358055156");

  I.seeInField("#numberOfGuests", "2");

  I.seeInField("#reservationTime", "18:00");

  I.seeInField("#note", "Sinh nhật, gần cửa sổ");

  I.seeElement("#reservationDate");
});

Scenario("TC11 - Kiểm tra thay đổi số lượng khách", async ({
  I,
}) => {
  I.amOnPage(BOOKING_URL);
