...
Scenario("TC02 - Kiểm tra hiển thị đủ 5 ngôi sao", async ({ I }) => {
  ...
  const count = await I.grabNumberOfVisibleElements("#starRating i");
...
Scenario("TC03 - Kiểm tra đánh giá mặc định là 5 sao", async ({ I }) => {
  ...
  for (let i = 1; i <= 5; i++) {
    I.seeElement(`#starRating i[data-value="${i}"].active`);
  }
});
...
Scenario("TC04 - Kiểm tra chọn 1 sao", async ({ I }) => {
  ...
  I.seeElement('#starRating i[data-value="1"].active');
  I.dontSeeElement('#starRating i[data-value="2"].active');
  I.dontSeeElement('#starRating i[data-value="3"].active');
  I.dontSeeElement('#starRating i[data-value="4"].active');
  I.dontSeeElement('#starRating i[data-value="5"].active');
});
...
Scenario("TC05 - Kiểm tra chọn 3 sao", async ({ I }) => {
  ...
  I.seeElement('#starRating i[data-value="1"].active');
  I.seeElement('#starRating i[data-value="2"].active');
  I.seeElement('#starRating i[data-value="3"].active');
  I.dontSeeElement('#starRating i[data-value="4"].active');
  I.dontSeeElement('#starRating i[data-value="5"].active');
});
...
Scenario("TC07 - Kiểm tra nhập tiêu đề đánh giá", async ({ I }) => {
  ...
  I.fillField("#reviewTitle", "Nhà hàng rất tuyệt");

  I.seeInField("#reviewTitle", "Nhà hàng rất tuyệt");
});
...
Scenario("TC10 - Kiểm tra nút Gửi đánh giá", async ({ I }) => {
  ...
  I.seeElement('#reviewForm button[type="submit"]');
  I.see("Gửi đánh giá");
});

Scenario("TC11 - Kiểm tra form đánh giá khi chưa nhập dữ liệu", async ({
  I,
}) => {
  I.amOnPage(REVIEW_URL);

  const title = await I.grabValueFrom("#reviewTitle");
  const content = await I.grabValueFrom("#reviewContent");
...
Scenario("TC12 - Kiểm tra nhập đầy đủ thông tin đánh giá", async ({ I }) => {
  ...
  I.fillField("#reviewTitle", "Dịch vụ xuất sắc");
  ...
  I.seeInField("#reviewTitle", "Dịch vụ xuất sắc");
  ...
  I.seeElement('#starRating i[data-value="5"].active');
});
...
Scenario("TC13 - Kiểm tra gửi đánh giá khi chưa đăng nhập", async ({ I }) => {
  ...
  I.fillField("#reviewTitle", "Đánh giá thử nghiệm");
...
Scenario("TC16 - Kiểm tra thay đổi nhiều mức đánh giá", async ({ I }) => {
  ...
  I.click('#starRating i[data-value="2"]');

  I.seeElement('#starRating i[data-value="1"].active');
  I.seeElement('#starRating i[data-value="2"].active');
  I.dontSeeElement('#starRating i[data-value="3"].active');

  I.click('#starRating i[data-value="4"]');

  I.seeElement('#starRating i[data-value="1"].active');
  I.seeElement('#starRating i[data-value="2"].active');
  I.seeElement('#starRating i[data-value="3"].active');
  I.seeElement('#starRating i[data-value="4"].active');
  I.dontSeeElement('#starRating i[data-value="5"].active');
});